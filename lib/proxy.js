...
ReverseProxy.prototype._createMitmProxy = function(src, target, opts) {
  ...
  const optsKey = _.map(_.pick(opts, ['changeOrigin']), mapper).join('/');

  const proxyKey = `${srcKey}=>${tgtKey}(${optsKey})`;
  // let proxy = this._createProxy({
  //   ...this.opts,
  //   proxyKey,
  //   ..._.pick(opts, ['targetSSL', 'changeOrigin']),
  // });
  proxy = this.proxy;
...
ReverseProxy.prototype.register = function(src, target, opts) {
  ...
  if (!route) {
    ...
    // Need a separate proxy for HTTPS MITM forward
    // if (opts.changeOrigin) {
    //   proxy = this._createMitmProxy(src, target, opts);
    // } else {
    //   proxy = this.proxy;
    // }
