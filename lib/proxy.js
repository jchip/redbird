...
ReverseProxy.prototype._readTargetFile = function(req, filename, target) {
  ...
  if (acceptEncoding) {
    ...
    promise = [encodings.includes('br') && 'br', encodings.includes('gzip') && 'gz']
      .filter(x => x)
      .reduce((pr, ec) => {
        return pr.then(() => {
          if (foundStat || cached) {
          ...
          const name = `${filename}.${ec}`;
          if (target.cache[name]) {
            cached = target.cache[name];
            return;
          }
          return fs.promises
            .stat(name)
  ...
  return promise
    .then(() => {
      return cached || foundStat || fs.promises.stat(foundName);
    })
    .then(stat => {
      return (
        cached ||
...
ReverseProxy.prototype._returnFile = function(req, res, route, target) {
  ...
  if (target.stat.isDirectory()) {
    filename = path.join(target.path, req.url);
  ...
  return this._readTargetFile(req, filename, target).then(file => {
    if (file.error) {
