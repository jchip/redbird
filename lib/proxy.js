...
/**
 * Generate a hash-based filename for temp compressed files
 */
...
/**
 * Check if temp compressed file exists and is newer than original
 */
async function checkTempFile(tempPath, originalStat) {
  try {
    const tempStat = await fs.promises.stat(tempPath);
    return tempStat.mtime > originalStat.mtime;
  } catch (error) {
    return false;
  }
}

ReverseProxy.prototype._readTargetFile = function(req, filename, target) {
  let foundStat;
    ...
    .then(originalStat => {
      ...
      if (acceptEncoding) {
        ...
        const supportedEncodings = [
          ...
          encodings.includes('gzip') && 'gz'
        ].filter(x => x);

        // Store the most preferred encoding for on-the-fly compression
        preferredEncoding = supportedEncodings[0] || '';

        // Only look for the preferred encoding to avoid falling back to other cached versions
        if (preferredEncoding) {
          const name = `${filename}.${preferredEncoding}`;
          const cacheKey = `${filename}.${preferredEncoding}`;
          if (target.cache[cacheKey]) {
            // Check if cached version is still valid by comparing with original file timestamp and age
            const cachedFile = target.cache[cacheKey];
            const now = Date.now();
            const cacheAge = now - cachedFile.timeStamp;

            if (
              cachedFile &&
              cachedFile.stat &&
              cachedFile.stat.mtime >= originalStat.mtime &&
              cacheAge < FIVE_MINUTES
            ) {
              // Cache is valid and not expired
              cached = cachedFile;
            } else {
              // Cache is invalid (outdated or expired), delete it and continue with compression logic
              delete target.cache[cacheKey];
            }
          } else {
            // First check for pre-compressed files
            promise = fs.promises
              .stat(name)
              .then(stat => {
              ...
              .catch(() => {
                // If no pre-compressed file, check temp compressed files
                const tempPath = getTempFilename(filename, preferredEncoding);
                return checkTempFile(tempPath, originalStat).then(isValid => {
                  if (isValid) {
                    encoding = preferredEncoding;
                    foundName = tempPath;
                    foundStat = originalStat; // Use original stat for cache validation
                    useTemp = true;
                  }
                });
              });
      ...
      return promise.then(() => {
        return cached || foundStat || originalStat;
      });
    })
    .catch(error => {
      // File doesn't exist, return error for proper 404 handling
      return { error };
    })
    .then(stat => {
      ...
      return (
        cached ||
        fs.promises.readFile(foundName).then(data => {
          const extName = path.extname(filename);
          const mimeType = extName && mime.getType(extName);

          // If we used a temp file, we already have compressed data
          if (useTemp) {
            const cacheKey = `${filename}.${encoding}`;
          ...
          // If we have a preferred encoding but no pre-compressed file was found, compress on-the-fly
          if (preferredEncoding && !encoding && foundName === filename) {
            return compressData(data, preferredEncoding)
              .then(compressedData => {
                // Save compressed data to temp file
                const tempPath = getTempFilename(filename, preferredEncoding);
                fs.promises.writeFile(tempPath, compressedData).catch(() => {
          ...
          const cacheKey = encoding ? `${filename}.${encoding}` : foundName;
          return (target.cache[cacheKey] = {
            name: filename,
            stat,
            data,
...
ReverseProxy.prototype._returnFile = function(req, res, route, target) {
  let filename;
  ...
  return this._readTargetFile(req, filename, target)
    .then(file => {
      if (file.error) {
        const errMsg = file.error.toString().replace(target.path, '');
        respondNotFound(req, res, errMsg);
      } else {
        if (file.mimeType) {
  ...
  const httpsServer = https.createServer(ssl, (req, res) => {
    ...
    this._getRouteTarget(src, req, res)
      .then(({ route, target }) => {
        ...
        if (target.protocol === 'file:') {
          if (!target.stat) {
            return fs.promises.stat(target.path).then(_stat => {
              target.stat = _stat;
              if (_stat.isDirectory()) {
                target.path = path.join(target.path, '/'); // add trailing slash
              }
