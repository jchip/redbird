...
// // maintain different proxies for src/target forwarding
// // a src will remember the key to lookup its proxy from the proxies
// const PROXY_KEY = Symbol('redbird.proxy.key');
...
function ReverseProxy(opts) {
  ...
  if (logOpts !== false) {
    log = this.log = pino(Object.assign({ name: 'redbird' }, logOpts));
  }
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    cluster.on('exit', function (worker, code, signal) {
      ...
      log &&
        log.error({ code: code, signal: signal }, 'worker died un-expectedly... restarting it.');
      cluster.fork();
    });
  } else {
    this._connections = {};
    ...
    //
    // Maintain different proxies base on these criteria:
    // - src protocol + target protocol
    // - target hostname + target port
    //
    this._proxies = {};

    const proxy = this._createProxy(opts);

    //
    // Plain HTTP Proxy
    //
    var server = this.setupHttpProxy(
      proxy,
      (...args) => this._websocketsUpgrade(...args),
      log,
      opts
    );
...
ReverseProxy.prototype._createProxy = function (opts) {
  ...
  //
  if (opts.ssl) {
    const websocketsUpgrade = (...args) => this._websocketsUpgrade(...args);
    if (_.isArray(opts.ssl)) {
      opts.ssl.forEach((sslOpts) => {
        this.setupHttpsProxy(proxy, websocketsUpgrade, this.log, sslOpts);
      });
    } else {
      this.setupHttpsProxy(proxy, websocketsUpgrade, this.log, opts.ssl);
    }
  }
...
ReverseProxy.prototype._websocketsUpgrade = function (req, socket, head) {
  ...
  this._getRouteTarget(src, req).then(function ({ target }) {
...
ReverseProxy.prototype.handleProxyError = function (err, req, res) {
  ...
  if (err.message !== 'socket hang up') {
    this.log && this.log.error(err, 'Proxy Error');
  }
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  this.server = httpServerModule.createServer((req, res) => {
    let _target, _route;
    var src = this._getSource(req);
    ...
    this._getRouteTarget(src, req, res)
      .then(({ target, route }) => {
        ...
        if (shouldRedirectToHttps(this.certs, src, target, this)) {
          return redirectToHttps(req, res, target, opts.ssl, log);
        } else {
  ...
  //
  this.server.on('upgrade', websocketsUpgrade);

  this.server.on('error', function (err) {
    log && log.error(err, 'Server Error');
  });

  this.server.on('connection', (conn) => this._saveConnection(conn));
...
ReverseProxy.prototype.register = function (src, target, opts) {
  ...
  if (!route) {
    ...
    if (target.protocol === 'https') {
      ...
      // const srcKey = _.map(_.pick(src, fields), mapper).join('/');
      // const tgtKey = _.map(_.pick(target, fields), mapper).join('/');

      // const proxyKey = `${srcKey}=>${tgtKey}`;
      // proxy = this._createProxy({ ...this._setupProxyEvents, proxyKey });
      proxy = this.proxy;
    } else {
      proxy = this.proxy;
    }
    // const proxyKey = this.makeProxyKey()
